...
export default function LessonScreen() {
  ...
  const [selectedAnswer, setSelectedAnswer] = useState<string | number | boolean | undefined>(
    undefined
  )
  ...
  function checkAnswer(answer: string | number | boolean) {
    ...
    if (
      ...
      correct =
        answer.toString().toLowerCase() === currentQuestion.correctAnswer?.toString().toLowerCase()
    }
      ...
      <ThemedView className="rounded-2xl p-6 shadow-lg">
        ...
        <View className="mt-6 rounded-lg bg-blue-50 p-4 dark:bg-blue-900">
          <ThemedText className="text-center text-blue-800 text-sm dark:text-blue-200">
            💡 Take a moment to read and understand this verse before moving to the exercises.
          </ThemedText>
        </View>
      </ThemedView>
    )
  ...
  function renderSummaryStep() {
    const percentage = Math.round((progress.score / (progress.totalSteps - 1)) * 100)
    ...
    return (
      <ThemedView className="rounded-2xl p-6 shadow-lg">
        ...
        <ThemedText className="mb-4 text-center font-bold text-2xl">Lesson Complete!</ThemedText>
        <ThemedText className="mb-6 text-center text-lg">{lessonData.title}</ThemedText>
        ...
        <View className="space-y-4">
          ...
          <View className="flex-row items-center justify-between rounded-lg bg-purple-50 p-4 dark:bg-purple-900">
            <ThemedText className="font-medium text-base">Time Spent</ThemedText>
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-gray-900">
      <ConfettiAnimation isVisible={showConfetti} onComplete={handleFinishLesson} />
